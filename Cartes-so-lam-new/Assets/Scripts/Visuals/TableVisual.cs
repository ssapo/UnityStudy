...
public class TableVisual : MonoBehaviour
{
	...
	public static bool CursorOverSomeTable
	{
		get
		{
			var bothTables = GameObject.FindObjectsOfType<TableVisual>();
	...
	private void Update()
	{
		...
		var passedThroughTableCollider = false;
	...
	public void AddCreatureAtIndex(CardAsset ca, int UniqueID, int index)
	{
		...
		var creature = Instantiate(GlobalSettings.Instance.CreaturePrefab, slots.Children[index].transform.position, Quaternion.identity) as GameObject;

		// apply the look from CardAsset
		var manager = creature.GetComponent<OneCreatureManager>();
		manager.cardAsset = ca;
		manager.ReadCreatureFromAsset();

		// add tag according to owner
		foreach (var t in creature.GetComponentsInChildren<Transform>())
		{
		...
		var w = creature.GetComponent<WhereIsTheCardOrCreature>();
		w.Slot = index;
		w.VisualState = VisualStates.LowTable;

		// add our unique ID to this creature
		var id = creature.AddComponent<IDHolder>();
	...
	public int TablePosForNewCreature(float MouseX)
	{
		...
		for (var i = 0; i < CreaturesOnTable.Count; i++)
		{
	...
	public void RemoveCreatureWithID(int IDToRemove)
	{
		...
		//    });
		var creatureToRemove = IDHolder.GetGameObjectWithID(IDToRemove);
	...
	{
		foreach (var g in CreaturesOnTable)
		{
