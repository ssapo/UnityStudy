...
public class HandVisual : MonoBehaviour
{
	...
	private void PlaceCardsOnNewSlots()
	{
		foreach (var g in CardsInHand)
		{
			...
			var w = g.GetComponent<WhereIsTheCardOrCreature>();
			if (w == null)
			{
	...
	private GameObject CreateACardAtPosition(CardAsset c, Vector3 position, Vector3 eulerAngles)
	{
		...
		else
		{
			...
			else
			{
				...
				var dragSpell = card.GetComponentInChildren<DragSpellOnTarget>();
		...
		var manager = card.GetComponent<OneCardManager>();
		manager.cardAsset = c;
	...
	public void GivePlayerACard(CardAsset c, int UniqueID, bool fast = false, bool fromDeck = true)
	{
		...
		foreach (var t in card.GetComponentsInChildren<Transform>())
		{
		...
		var w = card.GetComponent<WhereIsTheCardOrCreature>();
		w.BringToFront();
		w.Slot = 0;
